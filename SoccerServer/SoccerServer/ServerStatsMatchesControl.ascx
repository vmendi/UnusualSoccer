<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="ServerStatsMatchesControl.ascx.cs" Inherits="SoccerServer.ServerStatsMatchesControl" %>
...
<asp:GridView ID="MyMatchGridView" runat="server" AutoGenerateColumns="false" AllowPaging="true" 
    ...
    <Columns>
		...
		<asp:TemplateField HeaderText="Duration" ItemStyle-Width="40">
			<ItemTemplate>
				<asp:Label ID="Label3" runat="server" Text="<%# GetDurationOfMatch((SoccerServer.BDDModel.Match)Container.DataItem) %>" />
			</ItemTemplate>
		</asp:TemplateField>

        <asp:TemplateField HeaderText="Player1" ItemStyle-Width="200">
			<ItemTemplate>
				<asp:HyperLink ID="Label4" runat="server" Text="<%# GetPlayerNameOfMatch((SoccerServer.BDDModel.Match)Container.DataItem, 0) %>"
                                                          NavigateUrl="<%# GetProfileLinkOfMatch((SoccerServer.BDDModel.Match)Container.DataItem, 0) %>" />
			</ItemTemplate>
		</asp:TemplateField>

		<asp:TemplateField HeaderText="Player2" ItemStyle-Width="200">
			<ItemTemplate>
				<asp:HyperLink ID="Label5" runat="server" Text="<%# GetPlayerNameOfMatch((SoccerServer.BDDModel.Match)Container.DataItem, 1) %>"
                                                          NavigateUrl="<%# GetProfileLinkOfMatch((SoccerServer.BDDModel.Match)Container.DataItem, 1) %>" />
			</ItemTemplate>
		</asp:TemplateField>

        <asp:TemplateField HeaderText="Goals Player1" ItemStyle-Width="30">
			<ItemTemplate>
				<asp:Label ID="Label4" runat="server" Text="<%# GetGoalsOfMatch((SoccerServer.BDDModel.Match)Container.DataItem, 0) %>" />
			</ItemTemplate>
		</asp:TemplateField>

        <asp:TemplateField HeaderText="Goals Player2" ItemStyle-Width="30">
			<ItemTemplate>
				<asp:Label ID="Label4" runat="server" Text="<%# GetGoalsOfMatch((SoccerServer.BDDModel.Match)Container.DataItem, 1) %>" />
			</ItemTemplate>
		</asp:TemplateField>
